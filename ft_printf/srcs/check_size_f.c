...
void	check_size_and_negative_float1(t_spec **el, int *size, t_float **numb)
{
	if ((*el)->plus == 1 || (*el)->negative == 1 || (*el)->probel == 1)
	{
		if ((*el)->negative == 1 && (*el)->plus == 1)
		{
			...
			(*el)->negative + (*el)->precision && (*el)->field != 0))
				(*size)++;
		}
		else
		{
			if (!((*el)->field > ft_intlen((*numb)->int_value) +
			(*el)->negative + (*el)->probel + (*el)->plus +
			(*el)->precision && (*el)->field != 0))
				(*size)++;
		}
	}
}
